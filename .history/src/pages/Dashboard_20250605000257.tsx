...
// import AdminDashboard from "./AdminDashboard"; // importe ton composant admin
  ...
  if (isEditingProfile) {
    return (
      <div className="flex flex-col min-h-screen">
        <Navbar />
        <main className="flex-1 container mx-auto px-4 py-8">
          <ProfileEditForm profile={profile} onUpdate={updateProfile} onCancel={() => setIsEditingProfile(false)} />
        </main>
        <Footer />
      </div>
    );
    ...
    <div className="flex flex-col min-h-screen">
      ...
      <main className="flex-1 container mx-auto px-4 py-8">
        <div className="mb-8">
          <h1 className="text-3xl font-bold">Tableau de bord</h1>
          <p className="text-gray-600">
            Bienvenue sur votre espace personnel Lys&Co, {profile?.first_name || profile?.email}
          </p>
        </div>
                    ...
                    <div className="grid md:grid-cols-2 gap-4">
                      {reservations.map((res) => (
                        <div
                          ...
                          className="border rounded-xl p-4 shadow-sm bg-white"
                        >
                          <p className="mb-1">Date : {formatDate(res.reservation_date)}</p>
                          <p className="mb-1">
                            Heure : {res.start_time ? `${res.start_time} - ${res.end_time}` : "Journée/Demi-journée"}
                          </p>
                          <p className="mb-1">
