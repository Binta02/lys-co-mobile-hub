...
const ServiceDetail: React.FC = () => {
  ...
  // const fullDayDates = reservedPeriods
  //   .filter(
  //     (range) =>
  //       range ===
  //       `[${range.slice(1, 11)} 09:00:00+00,${range.slice(1, 11)} 16:00:00+00)`
  //   )
  //   .map((range) => {
  //     const dateStr = range.match(/\d{4}-\d{2}-\d{2}/)?.[0];
  ...
  const fullDayDates = reservedPeriods.reduce((dates: Date[], range) => {
    ...
    // Enregistre les plages par date
    const entry = dates.find((d) => d.toISOString().split("T")[0] === day);
    if (entry) return dates; // déjà marquée comme full day

    const isMorning = start.getHours() === 9 && end.getHours() === 12;
    const isAfternoon = start.getHours() === 13 && end.getHours() === 16;

    // Vérifie si les deux périodes sont présentes pour cette date
    const hasMorning = reservedPeriods.some(
      (r) =>
        r.includes(`${day} 09:00:00+00`) && r.includes(`${day} 12:00:00+00`)
    );
    const hasAfternoon = reservedPeriods.some(
      (r) =>
        r.includes(`${day} 13:00:00+00`) && r.includes(`${day} 16:00:00+00`)
    );
                      ...
                      highlightDates={[
                        {
                          ...
                          "react-datepicker__day--fullreserved": fullDayDates,
                        },
                      ]}
                      dayClassName={(date) => {
