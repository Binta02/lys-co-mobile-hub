      ...
      await Promise.all(
        items.map(async (item, idx) => {
          ...
          if (
            ...
            let periodKey: "morning" | "afternoon" | "journée" = "journée"; // valeur par défaut
            ...
            if (period?.includes("09:00:00") && period?.includes("12:00:00")) {
              ...
              // fallback si autre chose (journée complète ?)
              start = "09:00";
              end = "16:00";
            } else {
              // si period est vide, essaie de détecter à partir du titre
              const times = item.title.match(/\d{2}:\d{2}/g) || [];
              if (times.length === 1) {
                start = times[0];
