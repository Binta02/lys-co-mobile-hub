...
const downloadICS = (title: string, date: string, hour: string) => {
  ...
  )}\nDESCRIPTION=Réservation chez Lys Prestige\nEND:VEVENT\nEND:VCALENDAR`;
...
const ServiceDetail: React.FC = () => {
                      ...
                      >
                        ...
                        {id === "formation-room" && (
                          <>
                            ...
                            >
                              Demi-journée{" "}
                              {isFullDayReserved && "(indisponible)"}
                            </option>
                          </>
                        )}
  ...
  onClick={() => {
    const period = getReservationPeriod();
    const label = `${service.title} — ${translateReservationType(modeReservation)}${
      modeReservation === "halfDay" ? ` (${translateReservationType(halfDayPeriod)})` : ""
    } — ${dateReservation} ${selectedHours.join(", ")}`;
    ...
    if (modeReservation === "hour" && selectedHours.length) {
      setCalendarInfo({
        title: service.title,
...
{calendarInfo && (
  <Button
    ...
    onClick={() => {
      downloadICS(calendarInfo.title, calendarInfo.date, calendarInfo.hour);
  ...
  >
    Ajouter à mon agenda
                  ...
                  // Panier classique pour les autres services (avec session)
                  <Button
                    className="w-full bg-lysco-turquoise"
                    onClick={() => {
                      addItem({
