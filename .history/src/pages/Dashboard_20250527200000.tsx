...
const Dashboard: React.FC = () => {
  ...
  useEffect(() => {
    const fetchReservations = async () => {
      if (!profile?.id) {
        console.log(
          "⛔ Pas de profile.id, impossible de charger les réservations"
        );
        return;
      }
      ...
      if (error) {
        ...
        // Vérifie si chaque réservation a bien un payment_intent_id
        data?.forEach((r) =>
          console.log(
            `Réservation ${r.id} : payment_intent_id =`,
            r.payment_intent_id
          )
        );
      }
  ...
  // ...dans la map d'affichage des réservations...
  {
    reservations.map((res) => {
      console.log(
        `Affichage réservation ${res.id} : payment_intent_id =`,
        res.payment_intent_id
      );
      return (
        <div key={res.id} className="border rounded-xl p-4 shadow-sm bg-white">
          {/* ...existing code... */}
          <p className="mb-1 text-xs text-gray-400">
            payment_intent_id: {res.payment_intent_id || "Aucun"}
          </p>
          {/* ...existing code... */}
        </div>
      );
  ...
  //   // 1. Remboursement Stripe via backend
  ...
  //         toast({
  //           title: "Erreur",
  //           description: "Le remboursement Stripe a échoué.",
  //           variant: "destructive",
  //         });
  //         return;
  //       }
  ...
  //       toast({
  //         title: "Erreur",
  //         description: "Erreur lors du remboursement Stripe.",
  //         variant: "destructive",
  //       });
  //       return;
  //     }
                ...
                <div className="grid md:grid-cols-2 gap-4">
                  {reservations.map((res) => (
                    ...
                    >
                      ...
                      <p className="mb-1">Type : {res.reservation_type}</p>
