  ...
  useEffect(() => {
    const fetchFullDayDates = async () => {
      ...
      const fullDays = Object.entries(reservationsByDay)
        ...
        .map(([day]) => new Date(day)); // ← ici on génère bien Date[]
                    ...
                    <div className="space-y-2">
                      ...
                      {/* <DatePicker
                        ...
                        onChange={(date: Date | null) => {
                          ...
                          if (
                            date &&
                            fullDayDates.some(
                              (d) => localISO(d) === localISO(date)
                            )
                          ) {
                            // Ignore la sélection si la date est bloquée
                            return;
                          }
                          setDateReservation(
                            date ? date.toISOString().split("T")[0] : ""
                      ...
                      <DatePicker
                        ...
                        highlightDates={[
                          {
                            dates: reservedDates,
                            className: "react-datepicker__day--reserved",
                          },
                          {
                            dates: fullDayDates,
                            className: "react-datepicker__day--fullreserved",
                          },
                        ]}
                        filterDate={(date) => {
                          const day = date.getDay();
                          // Désactive juste les week-ends
                          if (day === 0 || day === 6) return false;
                          return true;
                        }}
                        dateFormat="dd/MM/yyyy"
