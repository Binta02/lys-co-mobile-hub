...
const Dashboard: React.FC = () => {
  ...
  useEffect(() => {
    if (!profile?.email) return; // ou profile?.id si tu as le client_id

    const fetchDocuments = async () => {
      setLoadingDocuments(true);
      try {
        const res = await fetch(
          `https://mon-backend-node.vercel.app/api/axonaut/documents?client_id=${profile.email}`
        );
        const data = await res.json();
        if (!res.ok) throw new Error(data.error || "Erreur API Axonaut");
        setDocuments(data); // ou data.data selon la réponse
      } catch (err: any) {
        console.error("❌ Erreur dans documents.ts :", err); // LOG DANS VERCEL
        return res.status(500).json({
          error: "Erreur lors de la communication avec Axonaut",
