...
const ServiceDetail: React.FC = () => {
  ...
  const [disabledDates, setDisabledDates] = useState<string[]>([]); // ✅ déclaré ici
  ...
  // }, [dateReservation, id]);
  // ⛔️ NE PAS mettre de useEffect ici !
  useEffect(() => {
    const fetchReservedPeriods = async () => {
  ...
  // ✅ Ce useEffect est maintenant en dehors et valide
  useEffect(() => {
    const fullReservedDates: string[] = [];

    const uniqueDates = new Set(reservedPeriods.map((p) => p.slice(1, 11)));

    uniqueDates.forEach((date) => {
      const morning = `[${date} 09:00:00+00,${date} 12:00:00+00)`;
      const afternoon = `[${date} 13:00:00+00,${date} 16:00:00+00)`;

      if (
        reservedPeriods.includes(morning) &&
        reservedPeriods.includes(afternoon)
      ) {
        fullReservedDates.push(date);
      }
    });

    setDisabledDates(fullReservedDates);
  }, [reservedPeriods]);
                    ...
                    <DatePicker
                      ...
                      dayClassName={(date) => {
                        ...
                        return disabledDates.includes(iso)
                          ? "disabled-day" // ta classe personnalisée pour afficher la croix
                          : "";
                      }}
                    />
                  </div>
