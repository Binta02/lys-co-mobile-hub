...
const Checkout = () => {
  ...
  const handleSubmit = async (data: FormValues) => {
    ...
    try {
      ...
      console.log("🎴 Création de PaymentMethod Stripe...");
      const { error: pmError, paymentMethod } =
        await stripe.createPaymentMethod({
          type: "card",
          card,
      ...
      console.log("🧾 Items panier:", items);
      ...
      const oneTimeItems = items
        ...
        .filter((i) => !subscriptionProductIds.includes(items[i.index].id));
      console.log("🛍️ oneTimeItems:", oneTimeItems);
      ...
      if (!resp.ok) {
        const text = await resp.text();
        console.error("❌ Réponse backend NOK:", text);
        throw new Error("Backend error: " + text);
      }
      ...
      for (const { clientSecret, index } of oneTimePaymentIntents) {
        console.log(`💳 Paiement one-time pour l'item #${index}...`);
        const { error: confirmErr } = await stripe.confirmCardPayment(
          clientSecret,
          {
            payment_method: paymentMethod.id,
        ...
        console.log(`✅ Paiement réussi pour item #${index}`);
      }
      ...
      if (subscriptionClientSecret) {
        ...
        console.log("✅ Paiement abonnement réussi");
      }

      console.log("📝 Insertion en base Supabase...");
      await Promise.all(
        items.map(async (item, idx) => {
          console.log(`📦 Insertion en base pour item #${idx}`, item);
          const isRoom =
            /(location-bureau|formation-room|coworking-space)/.test(item.id);
          const isDomiciliation = item.id.includes("domiciliation");

          if (isRoom) {
            console.log("🏢 Réservation de salle détectée");
            const date = item.id.match(/\d{4}-\d{2}-\d{2}/)?.[0];
            if (!date) {
              console.error("❌ Date non extraite pour :", item);
              return;
            ...
            console.log("📥 Insert réservation:", insertData);
            const { error } = await supabase
              .from("reservations")
              .insert(insertData);
            if (error) console.error("❌ Erreur réservation:", error);
            ...
            console.log("🏢 Insert domiciliation:", insertData);
            const { error } = await supabase
              .from("user_domiciliations")
              .insert(insertData);
            if (error) console.error("❌ Erreur domiciliation:", error);
            ...
            console.log("🛠️ Insert service:", insertData);
            const { error } = await supabase
              .from("user_services")
              .insert(insertData);
            if (error) console.error("❌ Erreur service:", error);
