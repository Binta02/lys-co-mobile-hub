...
export default function AdminDashboard() {
  ...
  const handleUpload = async () => {
    ...
    try {
      const response = await fetch("https://mon-backend-node.vercel.app/api/send-document-notification", {
        method: "POST",
      ...
      if (!response.ok) {
        ...
        console.error("❌ Erreur envoi e-mail :", err.error || response.statusText);
      } else {
        console.log("✅ E-mail de notification envoyé à", selectedClient.email);
      }
    } catch (err) {
      console.error("❌ Exception lors de l’appel à send-document-notification :", err);
    }
  };
    }
  };
