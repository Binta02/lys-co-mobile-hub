...
const MobileMenu: React.FC<MobileMenuProps> = ({ onClose, session }) => {
  ...
  return (
    <div className="md:hidden fixed inset-0 z-50 bg-white animate-fade-in">
      <div className="container mx-auto px-4 py-6 flex flex-col space-y-6 bg-white">
        ...
        <Link
          to="/"
          className="text-lg font-medium text-gray-700 hover:text-lysco-turquoise py-2"
          onClick={onClose}
        >
          Accueil
        </Link>
          ...
          {session ? (
            <>
              <Link to="/dashboard" onClick={onClose}>
                <Button
                  variant="outline"
                  className="w-full border-lysco-turquoise text-lysco-turquoise hover:bg-lysco-turquoise hover:text-white"
                >
                  Dashboard
                </Button>
              </Link>
