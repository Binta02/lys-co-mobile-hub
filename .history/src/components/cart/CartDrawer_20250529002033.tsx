...
export function CartDrawer({ onCheckout }: { onCheckout?: () => void }) {
  ...
  return (
    <div
      ...
      w-full max-w-[95vw] xs:max-w-xs sm:max-w-sm sm:w-[540px] flex flex-col
      relative
    "
    >
      {/* Bouton de fermeture */}
      <button
        ...
        className="absolute top-2 right-2 sm:top-4 sm:right-4 rounded-full p-2 bg-white hover:bg-gray-100 z-50"
      ...
      {items.length === 0 ? (
        ...
        <>
          ...
          <div className="pt-4 mt-4 border-t sticky bottom-0 bg-background">
            ...
            <div className="flex justify-between font-medium mt-2 pt-2 border-t text-sm sm:text-base">
              <span>Total</span>
              <span>{total.toFixed(2)} €</span>
            </div>
            <Button
              className="w-full mt-4 bg-lysco-turquoise hover:bg-lysco-turquoise/90 text-sm sm:text-base"
              asChild
              onClick={onCheckout}
            >
              <Link to="/checkout">Passer la commande</Link>
